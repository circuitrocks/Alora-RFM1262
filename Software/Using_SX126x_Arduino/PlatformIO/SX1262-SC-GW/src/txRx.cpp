...
// Show received message status on OLED display
#if OLED >= 1
#if OLED == 2 // 128x64 display
	char dispBuff[128];
	dispWriteHeader();

	int strWidth = display.getStringWidth(WiFi.localIP().toString());
	display.drawString(64 - (strWidth / 2), 11, WiFi.localIP().toString());

	sprintf(dispBuff, "%02i:%02i:%02i", hour(), minute(), second());
	display.drawString(0, 21, dispBuff);

	snprintf(dispBuff, 128, "%02X%02X%02X%02X", message[4], message[3], message[2], message[1]);
	display.drawString(64, 21, dispBuff);

#ifdef CFG_sx1262_radio
		display.drawString(0, 31, "RSSI: " + String(prssi));
#else
		display.drawString(0, 31, "RSSI: " + String(prssi - rssicorr));
#endif

	display.drawString(64, 31, "SNR: " + String(SNR));

	display.drawString(0, 51, "U " + String(statc.msg_ok + 1));
	display.drawString(64, 51, "D " + String(statc.msg_down));
	...
	char devAddr[128];
	snprintf(devAddr, 128, "%02X%02X%02X%02X", message[4], message[3], message[2], message[1]);
	display.drawString(0, 11, String(timBuff));
	display.drawString(64, 11, devAddr);
#ifdef CFG_sx1262_radio
	...
	display.drawString(64, 21, "SNR " + String(SNR));
